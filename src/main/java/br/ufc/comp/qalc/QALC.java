...
import br.ufc.comp.qalc.report.ErrorReporter;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (cmd.isUsageHelpRequested()) {
                ...
                //InputStream inputStream = (qalc.readFrom == null)? System.in : new FileInputStream(qalc.readFrom);
                ...
                MessageCenter.registerConsumerFor(MessageCategory.ERROR, new ErrorReporter(System.out, qalc.outputVerbosity));
                ...
                if (qalc.stopAt.ordinal() >= InterpreterPass.LEXER.ordinal()) {
                    ...
                    Scanner scanner = new Scanner(new Source(qalc.readFrom));
                    NewTokenMessage newTokenMessage = new NewTokenMessage(scanner.getNextToken());

                    while(!(newTokenMessage.getToken() instanceof EOFToken)){
                        if( !newTokenMessage.getToken().getTokenIdentifier().equals("WHITE") &&
                            !newTokenMessage.getToken().getTokenIdentifier().equals("COM")){//ignora commentarios e whitespace
                            MessageCenter.deliver(newTokenMessage);
                        }
                        if(newTokenMessage.getToken().getTokenIdentifier().equals("ERROR")){
                            MessageCenter.deliver(new ErrorTokenMessage(newTokenMessage.getToken(), "Token não reconhecido"));
                        }

                        newTokenMessage = new NewTokenMessage(scanner.getNextToken());
                    }
